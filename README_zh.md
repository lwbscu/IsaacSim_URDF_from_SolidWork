...
1. 通过 `./run.sh` 启动界面。
2. 选择**目标所在文件夹路径**：点击“浏览”选择资产的根目录。
3. 选择**原 URDF 路径**：点击“浏览”定位到原始的 `.urdf` 文件（工具具备智能推断能力，会自动帮你补全根目录）。
4. 输入**期望改名后的名称**：输入纯净的资产名（如 `robot_v2`，仅限纯英文与下划线，无需加 `.urdf` 后缀）。
5. 点击 `2. 一键清洗重构 (Clean)`。
6. 打开 NVIDIA Isaac Sim -> 点击菜单栏 **File** -> **Import** -> 选中新生成的 `.urdf` 文件即可导入。
